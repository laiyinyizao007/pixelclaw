...
当上述内置恢复均无效、脚本抛出未处理异常时，自动调用 **Claude Haiku** 分析当前 UI 状态并给出恢复动作：

- **触发条件**：`except Exception` 块捕获到异常（内置恢复已先行尝试）
- **行为**：截取当前 UIAutomator XML + 错误信息 → 发给 Haiku → 执行返回的 `back`/`tap`/`skip` 动作
- **依赖**：环境变量 `ANTHROPIC_API_KEY` 必须存在；缺少时自动降级为跳过该职位
- **费用**：仅在出错时才调用，正常爬取 0 token 消耗

```
日志示例：
  ERROR  处理职位 'xxx' 时出现异常
  INFO   [AI recovery] 执行 Back
```

### 返回列表优化（Back 键快速通道）

每条职位爬完后，脚本先尝试 **Back 键**直接返回列表（约 1.2s），检测到列表后即可开始下一条，无需重新搜索。仅当 Back 落到意外页面时才回退为完整重搜流程。

实测 100 条职位节省约 **10–12 分钟**（原重搜路径约 7s/条 × 100 条）。
